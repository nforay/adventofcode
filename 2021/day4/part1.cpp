...
int main()
{
	...
	std::cerr << "Draw: ";
	while (!winner && std::getline(ss, line, ','))
	{
		draw = std::stoi(line);
		std::cerr << draw << ", ";
		drawn[draw] = true;
		for (int i = 0; i < grids.size() && !winner; i += 25)
	...
	std::cerr << "Sum of unmarked numbers: " << score << std::endl;
	std::cout << "Answer: " << draw * score << std::endl;
	return (0);
}